...
class KinectCalibrator:
    def __init__(self, openni_path="C:/Program Files/OpenNI2/Redist", chessboard_size=(9, 6), square_size=0.024):
        ...
        self.objpoints_rgb = []
        self.objpoints_ir = []
        self.objpoints_depth = []
        self.imgpoints_rgb = []
        self.imgpoints_ir = []
        self.imgpoints_depth = []
        
        self.RGB_dir = "./Fotos_cal_RGB"
        self.IR_dir = "./Fotos_cal_IR"
        self.DEPTH_dir = "./Fotos_cal_DEPTH"
        ...
        self.DEPTH_imgs = glob.glob(self.DEPTH_dir + "/*.jpg")
        self.take_pictures_stereo = True
        self.take_pictures_depth = True

        if len(self.RGB_imgs) == len(self.IR_imgs) and len(self.RGB_imgs) >= 50:
            self.take_pictures_stereo = False
        if len(self.DEPTH_imgs) >= 50:
            self.take_pictures_depth = False        
    ...
    def add_corners_depth(self, depth_image):
        found, corners = cv2.findChessboardCorners(depth_image, self.chessboard_size, None)
        if found:
            criteria = (cv2.TERM_CRITERIA_EPS + cv2.TERM_CRITERIA_MAX_ITER, 30, 0.001)
            corners = cv2.cornerSubPix(depth_image, corners, (11, 11), (-1, -1), criteria)
            self.objpoints_depth.append(self.objp)
            self.imgpoints_depth.append(corners)
            return True
        return False
...
if __name__ == "__main__":
    ...
    image_shape_ir = None
    image_shape_depth = None
    mode = "DEPTH"

    match mode:
        case "STEREO":
            if calibrator.take_pictures_stereo:
        ...
        case "DEPTH":
            if calibrator.take_pictures_depth:
                calibrator.start_ir_stream()
                print("Toma de fotos activada.")
                print("Presiona 'c' para capturar imagen, 'n' para finalizar.")
                while calibrator.frame_count < 60:
                    depth = calibrator.capture_ir_frame()
                    print(f"Frame {calibrator.frame_count}")
                    depth = cv2.GaussianBlur(depth, (5, 5), 0)
                    cv2.imshow("DEPTH cam", depth)
                    key = cv2.waitKey(1) & 0xFF
                    if key == ord("c"):
                        if calibrator.add_corners_depth(depth):
                            print("Esquinas detectadas y agregadas en img DEPTH.")
                            filename = os.path.join(calibrator.DEPTH_dir, f"frame_{calibrator.frame_count:05d}.jpg")
                            cv2.imwrite(filename, depth)
                            calibrator.frame_count += 1
                            image_shape_depth = (depth.shape[1], depth.shape[0])
                        else:
                            print("No se detectaron esquinas en img DEPTH.")
                cv2.destroyAllWindows()                  
            else:
                index = 0    
                for img in calibrator.DEPTH_imgs:
                    depth = cv2.imread(calibrator.DEPTH_imgs[index], cv2.IMREAD_GRAYSCALE)
                    #depth = cv2.GaussianBlur(depth, (5, 5), 0)
                    if calibrator.add_corners_depth(depth):
                        print(f"DEPTH: Esquinas detectadas y agregadas frame = {index:05d}.")
                    else:
                        print(f"DEPTH: No se detectaron esquinas frame = {index:05d}, imagen removida del array.")
                        if calibrator.imgpoints_depth and calibrator.objpoints_depth:
                            calibrator.objpoints_depth.pop()
                            calibrator.imgpoints_depth.pop()
                    index += 1
                image_shape_depth = (depth.shape[1], depth.shape[0])

            print("Calibrando camaras individuales...")
            ret_depth, mtx_depth, dist_depth, rvec_depth, tvec_depth = calibrator.calibrate_camera(calibrator.objpoints_depth, calibrator.imgpoints_depth, image_shape_depth)
            rvec_depth = np.array(rvec_depth, dtype=np.float64)
            tvec_depth = np.array(tvec_depth, dtype=np.float64)
            rvec_depth_mean = np.array((np.mean(rvec_depth, axis=0))).reshape(3, 1)
            tvec_depth_mean = np.array((np.mean(tvec_depth, axis=0))).reshape(3, 1)
            print("rvec_depth_mean: ", rvec_depth_mean)
            #if rvec_depth.shape == (3,):
                #rvec_depth = rvec_depth.reshape(3, 1)
            
            rvec_depth_mean, _ = cv2.Rodrigues(rvec_depth_mean)
            
            print("Calibracion DEPTH Error:", ret_depth)
            print("mtx_depth: ", mtx_depth)
            print("dist_depth: ", dist_depth)
            print("rmtx_depth: ", rvec_depth_mean)
            print("tvec_depth: ", tvec_depth_mean)
            # print("objpoints DEPTH: ", len(calibrator.objpoints_rgb))
            # print("img points DEPTH: ", len(calibrator.imgpoints_rgb))


            with open("calibration.txt", "w") as file:
                file.write("ret_depth: " + str(ret_depth) + "\n")
                file.write("mtx_depth: " + str(mtx_depth) + "\n")
                file.write("dist_depth: " + str(dist_depth) + "\n")
                file.write("rmtx_depth: " + str(rvec_depth_mean) + "\n")
                file.write("tvec_depth: " + str(tvec_depth_mean) + "\n")
